...
const {stringify: toMetaString} = require('usercss-meta');
...
const Stylus = {
  ...
  format(meta) {
    ...
    return toMetaString(meta, {alignKeys: true, format: 'xstyle'});
  },
  ...
  mozDocument(rule, url) {
    const found = Object.values(this.RULES).find(r => (r === rule));
