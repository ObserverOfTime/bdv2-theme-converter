...
/**
 * Class representing a Value Error.
 *
 * @private
 * @extends {Error}
 */
...
const Stylus = {
  ...
  format: (meta) => {
    ...
    if(!semver.valid(meta.version))
      throw new ValueError(`UserStyle version (${ver}) doesn't follow SemVer`);
  ...
  mozDocument: (rule, url) => {
    ...
    if(!found) throw new ValueError(`Invalid @-moz-document rule '${rule}'`);
