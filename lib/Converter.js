...
class Converter {
  ...
  set paths(paths) {
    ...
    Object.defineProperty(this.paths, '_fullPath', {
      enumerable: false,
      configurable: false,
      writable: false,
      value: (key) => {
        const val = (key === 'src') ?
          this.paths.src : this.paths.dest[key];
        return path.resolve(this.paths.root, val);
      }
    });
  }
  ...
  toLegacyTheme(options={}) {
    this.compile((err, css) => {
      ...
      const dest = this.paths._fullPath('legacyTheme');
  ...
  toUserStyle(options={}, extras={}) {
    this.compile((err, css, indent) => {
      ...
      css = css.toString()
        .replace(/^/gm, indent)
        .replace(/^[\s\t]+$/gm, '');
      const dest = this.paths._fullPath('userStyle');
  ...
  toMinified(options={}, meta=false) {
    ...
    this.compile((err, css) => {
      if(err) return console.error(err.message);
      let dest = this.paths._fullPath('minified');
      if(meta) {
